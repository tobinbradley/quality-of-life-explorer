<script>
  ...
  function search () {
    ...
    if (isNumeric(str)) {
      ...
      if (keys.indexOf(str) !== -1) searchResults = searchResults.concat({ cat: 'NPA', id: str, label: str })

      // zipcode
      if (str.length === 5) {
        fetch(`https://api.mcmap.org/v1/query/zipcodes?columns=gid as id, zip as label, 'ZIP' as cat&limit=10&filter=zip='${str}'`)
    ...
    if (!isNumeric(str) && str.length >= 4) {
      ...
      // NSA
      fetch(`https://api.mcmap.org/v1/query/neighborhood_statistical_areas?columns=gid as id, nsa_name as label, 'NSA' as cat&limit=10&filter=nsa_name ilike '${str}%'`)
        .then(response => response.json())
        .then(json => {
          searchResults = searchResults.concat(json)
        })
  ...
  function selectResults(cat, id) {
    ...
    if (cat === 'ZIP') {
      fetch(`https://api.mcmap.org/v1/intersect_feature/zipcodes/neighborhoods?geom_column_from=the_geom&geom_column_to=the_geom&columns=neighborhoods.id&filter=zipcodes.gid=${id}`)
      .then(response => response.json())
      .then(json => {
        $selectedNeighborhoods = json.map(el => el.id.toString())
      })
    ...
    if (cat === 'ADDRESS') {
      fetch(`https://api.mcmap.org/v1/intersect_feature/master_address_table/neighborhoods?geom_column_from=the_geom&geom_column_to=the_geom&columns=neighborhoods.id&filter=master_address_table.objectid=${id}`)
      .then(response => response.json())
      .then(json => {
        $selectedNeighborhoods = json.map(el => el.id.toString())
      })
...
<div class="relative">
  ...
  <p class="text-sm text-stone-600">
    Enter a <span class="text-pink-800 hover:cursor-pointer"
    ...
    NPA</span>, address, NSA or zip code.
  </p>
</div>
