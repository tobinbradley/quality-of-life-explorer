...
const goNPA = async () => {
  for (const el of config) {
    ...
    // r and years
    if (
      fs.existsSync(path.join(dataSrc, `r${el.metric.replace("m", "")}.csv`))
    ) {
      data.r = {}
      const rJSON = await csv().fromFile(
        path.join(dataSrc, `r${el.metric.replace("m", "")}.csv`)
      )
      ...
      rJSON.forEach((rec, idx) => {
        ...
        objectVals.shift()
        data.r[rec.id] = objectVals.map((n) =>
          isNumeric(n) ? Number(n) : null
        )
      })
    }
    ...
    // d
    if (
      fs.existsSync(path.join(dataSrc, `d${el.metric.replace("m", "")}.csv`))
    ) {
      data.d = {}
      const rJSON = await csv().fromFile(
        path.join(dataSrc, `d${el.metric.replace("m", "")}.csv`)
      )

      rJSON.forEach((rec, idx) => {
        const objectVals = objectToArray(rec)
        objectVals.shift()
        data.d[rec.id] = objectVals.map((n) =>
          isNumeric(n) ? Number(n) : null
        )
      })
    }
    ...
    if (
      fs.existsSync(
        path.join(dataSrc, `m${el.metric.replace("m", "")}-accuracy.csv`)
      )
    ) {
      data.a = {}
      const rJSON = await csv().fromFile(
      ...
      rJSON.forEach((rec, idx) => {
        const objectVals = objectToArray(rec)
        objectVals.shift()
        data.a[rec.id] = objectVals.map((n) =>
          isNumeric(n) ? Number(n) : null
        )
      })
    }
      ...
      for (const key in data.r) {
        ...
        data.r[key].forEach((el, idx) => {
          if (isNumeric(el) && isNumeric(data.d[key][idx])) {
            mVals.push(Math.round((el / data.d[key][idx]) * 10000) / 10000)
          } else {
            mVals.push(null)
          }
        })
    ...
    fs.writeFileSync(
      path.join(dataDest, `${el.metric}.json`),
